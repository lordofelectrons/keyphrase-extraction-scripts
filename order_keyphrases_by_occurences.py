...
from transformers import (
    ...
    set_seed, TextClassificationPipeline,
)
...
def main():
    ...
    inputs_with_occurences = {}
    for i in range(len(raw_datasets["train"][text_column])):
        if raw_datasets["train"][text_column][i] and (
                raw_datasets["train"][extractive_keyphrases][i] or raw_datasets["train"][abstractive_keyphrases][i]):
            current_inputs = []
            current_inputs.extend([x for x in raw_datasets["train"][extractive_keyphrases][i] if x])
            current_inputs.extend([x for x in raw_datasets["train"][abstractive_keyphrases][i] if x])
            current_inputs = list(map(space_to_underscore, current_inputs))
            for kwrd in current_inputs:
                if kwrd in inputs_with_occurences:
                    inputs_with_occurences[kwrd] += 1
                else:
                    inputs_with_occurences[kwrd] = 1

    for i in range(len(raw_datasets["validation"][text_column])):
        if raw_datasets["validation"][text_column][i] and (
                raw_datasets["validation"][extractive_keyphrases][i] or raw_datasets["validation"][abstractive_keyphrases][i]):
            current_inputs = []
            current_inputs.extend([x for x in raw_datasets["validation"][extractive_keyphrases][i] if x])
            current_inputs.extend([x for x in raw_datasets["validation"][abstractive_keyphrases][i] if x])
            current_inputs = list(map(space_to_underscore, current_inputs))
            for kwrd in current_inputs:
                if kwrd in inputs_with_occurences:
                    inputs_with_occurences[kwrd] += 1
    ...
    print(inputs_with_occurences)
    underscore_to_space = lambda x: x.replace('_', ' ')
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and (examples[extractive_keyphrases][i] or examples[abstractive_keyphrases][i]):
                ...
                occurenced_keyphrases = {}
                for kphrs in list(map(space_to_underscore, targets_ext_non_null + targets_abs_non_null)):
                    occurenced_keyphrases[kphrs] = inputs_with_occurences[kphrs]
                targets.append(', '.join(list(map(underscore_to_space, sorted(occurenced_keyphrases)))))
