...
def main():
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and (examples[extractive_keyphrases][i] or examples[abstractive_keyphrases][i]):
                inputs_non_null = ' '.join([x for x in examples[text_column][i] if x])
                text = inputs_non_null.split(' . ')
                random.shuffle(text)
                inputs_non_null = ' . '.join(text)
                inputs.append(inputs_non_null)
                ...
                print({"keyphrases": keyphrases, "text": inputs_non_null})
                sys.stdout.flush()
    ...
    if training_args.do_train:
        ...
        train_dataset = concatenate_datasets([
            load_dataset('json', data_files='/gpfs/space/home/filipenk/tmp/train_inspec_synonyms_5000.json')["train"],
