...
def main():
    ...
    def preprocess_function(examples):
        ...
        def sort_ext_by_entry(ext_keyphrases, text):
            entries = {}
            for keyphrase in ext_keyphrases:
                index = text.find(keyphrase)
                entries[index] = keyphrase
            ordered = []
            for key in sorted(entries):
                ordered.append(entries[key])
            return ordered

        def insert_abs_randomly(ordered_ext, abs_keyphrases):
            for abs_k_index in range(len(abs_keyphrases)):
                index = random.randint(0, len(ordered_ext))
                ordered_ext = ordered_ext[:index] + [abs_keyphrases[abs_k_index]] + ordered_ext[index:]
            return ordered_ext
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and (examples[extractive_keyphrases][i] or examples[abstractive_keyphrases][i]):
                ...
                keyphrases = insert_abs_randomly(sort_ext_by_entry(targets_ext_non_null, re.sub("[^A-Za-z0-9 ]", "",
                                                    inputs[-1].lower())), targets_abs_non_null)
